...
pub_jackal_rear_left = rospy.Publisher('/rear_left_wheel_controller/command', Float64)
pub_jackal_rear_right = rospy.Publisher('/rear_right_wheel_controller/command', Float64)
pub_jackal_front_left = rospy.Publisher('/front_left_wheel_controller/command', Float64)
pub_jackal_front_right = rospy.Publisher('/front_right_wheel_controller/command', Float64)
...
rp=[0,3.14,0,0.5*math.pi,0,-math.pi*0.5*0.66,0]
...
while not rospy.is_shutdown():
	...
	jackal_msg = wheelVelocities[0]
	pub_jackal_front_left.publish(jackal_msg)

	jackal_msg = wheelVelocities[1]
	pub_jackal_front_right.publish(jackal_msg)

	jackal_msg = wheelVelocities[2]
	pub_jackal_rear_left.publish(jackal_msg)

	jackal_msg = wheelVelocities[3]
	pub_jackal_rear_right.publish(jackal_msg)

	baseorn = p.getQuaternionFromEuler([0,0,ang])
	for i in range(len(wheels)):
		p.setJointMotorControl2(jackal,wheels[i],p.VELOCITY_CONTROL,targetVelocity=wheelVelocities[i], force=100000)
	...
	# 	pos = [0.1*math.cos(t),0,0.+0.2*math.sin(t)+0.7]
	# 	#pos = [0.5,0.5,0.8]
	...
	# 			jointPoses = accurateCalculateInverseKinematics(jaco,jacoEndEffectorIndex,pos, threshold, maxIter)
		...
		# if (useSimulation):
		# 	for i in range (numJoints):
		# 		p.setJointMotorControl2(bodyIndex=jaco,jointIndex=i,controlMode=p.POSITION_CONTROL,targetPosition=jointPoses[i],targetVelocity=0,force=0,positionGain=1,velocityGain=0.1)
		# else:
